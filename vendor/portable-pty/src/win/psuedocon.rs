...
fn load_conpty() -> ConPtyFuncs {
    ...
    //
    // Winter patch: upstream opens the bare name "conpty.dll", which makes
    // LoadLibrary walk PATH and pick up whatever other application's private
    // ConPTY build happens to be there (WezTerm's, for one). That build
    // deadlocks in ClosePseudoConsole when a pane is dropped while its reader
    // thread is still blocked in ReadFile, which hung Winter on every launch.
    // Only accept a conpty.dll that sits next to our own executable, loaded
    // by absolute path; otherwise use the in-box ConPTY from kernel32.
    match sideloaded_conpty_path().and_then(|path| ConPtyFuncs::open(&path).ok()) {
        Some(sideloaded) => sideloaded,
        None => kernel,
    }
}

const SIDELOADED_CONPTY_DLL: &str = "conpty.dll";

/// Absolute path of a `conpty.dll` deployed beside the running executable,
/// if there is one.
fn sideloaded_conpty_path() -> Option<PathBuf> {
    let path = std::env::current_exe()
        .ok()?
        .parent()?
        .join(SIDELOADED_CONPTY_DLL);
