...
The motion primitives are pure functions over a `&Grid` with no `App` state involved, which is why they are cheap to test. A motion never lands on punctuation, which over code is a cursor stop nobody asks for: small-word motions (`w`, `b`, `e`) cross it the way they cross a blank, so `w` over `foo.bar` goes straight to the `b`, and big-word motions (`W`, `B`, `E`) take it into the run beside it, so `foo.bar` is one word. Text objects (`iw`, `aw`) classify the vim way instead, keyword runs and punctuation runs apart, since a selection that stops at a punctuation boundary is one worth having.

The spans themselves are pure functions over rows of characters, in the Vim foundation rather than in the grid: a row's words and quoted runs, and the bracket pairs, paragraphs, and sentences that may cross rows, reached through a small `TextRows` trait that the grid and the editor's buffer each answer. One implementation of `iw` serves both surfaces, so an object fixed in one is fixed in the other.
...
Not every pane is a terminal. A pane can instead hold a *tool page*: content Winter paints itself, as rows of styled text rather than PTY output. `crates/winter-term/src/model/page.rs` defines what a page is, one module per tool lives under `crates/winter-term/src/tools/`, and a page's own logic stays pure and std-only so it is testable without a window or a filesystem. A page names semantic styles (accent, dim, header, normal) and never a color, so the active theme resolves them.

A page covers a pane rather than replacing it: the map of open pages is keyed by `PaneId` the same way modes, nav cursors, and marks already are, and the terminal underneath goes on running, unseen, until the page closes and hands the pane back in the mode it was in. The split tree, tabs, focus, and zoom are untouched, since they only ever deal in ids, and nothing about a page reaches a session snapshot: the pane it covers is still an ordinary terminal. A page paints into a grid of its own, which then goes through the same per-pane view the renderer draws a terminal through, so a tool pane costs the renderer nothing new. It is handed its row count before it paints, so a listing longer than the pane returns the window it wants visible: scrolling belongs to the page, which is the only side that knows where its cursor is. A pane a page covers is one nothing else may draw into, so its rich-block tiles and images are held back for as long as the page is up, on the same rule that hides them under a full-screen app.

Slow work follows the same shape. A page asks for it, a detached worker thread does it, and the answer is collected on the next event-loop poll and handed back to the page, which then asks for the next piece. The work is either a program to run or a walk of the filesystem, and a walk is Winter's own rather than a shelled-out tool, so a directory total and a text search behave the same on every platform and every accumulator they fill has a cap. Requests past a fixed number in flight are dropped rather than queued, since every caller is asking about what is on screen and a queued answer would be about a screen the user has left. Cancelling forgets the job rather than killing the thread: the worker notices a flag and exits, and an answer that arrives anyway is discarded because the job is no longer one anybody is waiting on. While any job runs, the loop keeps its busy poll interval so a finished one is never left sitting in the channel.

A page may hold more than one thing at a time where that is what the thing is: the editor keeps a list of open files and shows one of them, so a second file opened while it is up joins it rather than covering it. The host offers the file to whatever page is up and falls back to opening a page of its own when it is declined, which keeps the rule that a page owns what is drawn in its pane.

A page can also be opened *over* another one rather than in place of it, which is how opening a file from a listing works: the covered page is pushed onto a per-pane stack, and closing the one on top pops it back rather than uncovering the terminal, so the listing is still where it was left. A page coming back is told so, and re-reads whatever it was showing, since a file saved over the top of it is exactly the kind of thing that changes what it says. Work the covered page had asked for is cancelled as it goes under, because the answer would otherwise come back to whichever page is now on top.

A page that needs an answer from the user asks for one rather than reading the keyboard itself: it returns a prompt request carrying a tag, the host collects the line in the status bar, and the tagged answer comes back to the page. A question whose wrong answer cannot be undone is collected as a single key instead of a line. That way one input path serves every tool, and a page never has to know what a status bar is.

Searching is shared on the same principle. `/` is a prompt like any other, and the answer goes to one matcher the pages hand their own rows to, so `/`, `n`, and `N` mean the same thing in every tool. The match is literal text rather than a pattern, which is what keeps the page layer free of the regex engine the scrollback search needs. Each tool says what text stands for one of its rows, usually the row as painted, so the key list is as searchable by chord as by command.

The pointer reaches a page the same way: a press, a drag, and a turn of the wheel are offered to the page over the pane before the grid underneath sees them, and a page that answers none of them leaves the terminal's own selection untouched. The editor is the one that takes all three, so a click lands the cursor on the character under it and a drag selects from where the button went down.
