...
impl App {
    ...
    /// The traversal cursor as an absolute `(row, col)`, which is how the
    /// Visual anchor and the `gv` snapshot both address text.
    pub(crate) fn absolute_nav_cursor(&self, pane: PaneId) -> Option<(usize, usize)> {
        let (row, col) = self.nav_cursor(pane)?;
        Some((self.panes.get(&pane)?.grid().to_absolute_row(row), col))
    }
    ...
    pub(crate) fn select_paragraph(&mut self, focused: PaneId) {
        ...
        self.selection.visual_anchor = Some((grid.to_absolute_row(start), 0));
    ...
    pub(crate) fn select_text_object(&mut self, focused: PaneId, around: bool, object: TextObject) {
        ...
        if let Some(pane) = self.panes.get(&focused) {
            ...
            self.selection.visual_anchor = Some((abs_r1, c1));
...
mod tests {
    ...
    /// A pane holding many pages of scrollback, so full-page motions run against
    /// real history rather than the live-bottom edge case. The lines are written
    /// straight into the grid (no shell, no timing): waiting on a shell to emit
    /// them made every test using this fixture fail on a loaded machine.
    ...
    /// How long a test waits for a child process to produce what it was asked
    /// for. Generous, because every wait below breaks out as soon as the output
    /// arrives: the ceiling only has to survive a machine under load.
    const PTY_WAIT: std::time::Duration = std::time::Duration::from_secs(5);

    /// How long to sleep between polls, so waiting does not spin a core.
    const POLL_STEP: std::time::Duration = std::time::Duration::from_millis(20);

    /// Write numbered lines into `grid` until well past its height, the shape
    /// `seq 1 200` used to produce here.
    fn fill_scrollback(grid: &mut winter_render::Grid) {
        for line in 1..=200 {
    ...
    fn test_page_down_from_history_does_not_strand_cursor_past_last_prompt() {
        ...
        // One content row and blank rows under it, written straight into the
        // grid: what the shell used to be asked for, without the wait.
        let pane = pane_with_lines(&["hi"]);
    ...
    fn test_dot_replays_the_last_insert_typing_run() {
        ...
        let deadline = std::time::Instant::now() + PTY_WAIT;
        while std::time::Instant::now() < deadline {
            app.panes.get_mut(&id).unwrap().drain_output();
