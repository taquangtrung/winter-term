...
/// Bounds for the pane divider. One logical pixel is the thinnest line that
/// still draws, and past a few the divider reads as a gap rather than a rule.
const MIN_PANE_BORDER: f32 = 1.0;
const MAX_PANE_BORDER: f32 = 8.0;
const PANE_BORDER_STEP: f32 = 1.0;
...
pub struct App {
    ...
    /// Every open tab and the state of the bar across the top of them (see [`TabsState`]).
...
mod tests {
    ...
    /// How long a test waits for a child process to do what it was asked. Every
    /// wait breaks out as soon as it happens, so the ceiling only has to survive
    ...
    /// `seq 1 200` used to produce here. The navigation tests keep their own
    /// copy, so neither test module has to reach into the other.
    ...
    /// A pane holding many pages of scrollback, so auto-scroll runs against real
