...
use crate::model::page::{
    ...
    PageSpan, PageStyle, PromptMode, PromptReply, PromptRequest, SpawnRequest,
};
...
impl GitPage {
    ...
    fn open(&self, path: &str) -> PageOutcome {
        ...
        PageOutcome::OpenPath(root.join(path))
    }
        ...
        match output.tag {
            ...
            exec::TAG_READ => {
                if output.stdout.trim().is_empty() {
                    ...
                    self.show_output("Output", &output.stdout, false);
                }
                PageOutcome::Consumed
            }
            exec::TAG_LOG_VIEW => {
