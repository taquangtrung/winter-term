...
pub struct BlockQueue {
    ...
    /// Indices of entries whose source block was elided since the last
    /// [`BlockQueue::take_elided`], so the app can drop the rendered block
    /// (texture, WebView tile) that can never show content again.
    elided: Vec<usize>,
...
/// A reserved band: where it starts and how tall it is.
///
/// The height travels with the anchor because the reservation is the single
/// decision about how much room a block gets. Re-deriving it downstream (the
/// renderer assuming the default band for a block the grid actually reserved
/// five rows for) leaves the drawn image and the blank rows disagreeing, which
/// shows up as dead space between the block and the next prompt.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct BlockBand {
    /// First row of the band (see [`winter_render::Grid::to_absolute_row`]).
    pub abs_row: usize,
    /// Rows the band occupies in the grid.
    pub rows: usize,
}
...
pub struct BlockEntry {
    /// Absolute row the block's reserved band starts at (see
    /// [`winter_render::Grid::to_absolute_row`]). Absolute rather than a
    /// screen row so the anchor keeps naming the band's own first line as
    /// output scrolls the grid underneath it.
    pub abs_row: usize,
    ...
    pub kind: BlockKind,
    /// Rows the grid reserved for this block's band. The renderer draws into
    /// exactly these, so the image and the blank rows behind it agree.
...
impl BlockQueue {
    ...
    /// `anchors` gives the band each new block was reserved at, in emission
    /// order; `fallback` is used if anchors run short.
    pub fn update(&mut self, scrollback: &Scrollback, fallback: BlockBand, anchors: &[BlockBand]) {
            ...
            for (segment_index, segment) in block.output.iter().enumerate().skip(start) {
                match segment {
                    Segment::Content(emit) => {
                        let band = anchors.next().copied().unwrap_or(fallback);
                        self.entries.push(BlockEntry {
                            abs_row: band.abs_row,
                            block_index,
                            closed: false,
                            emit: emit.clone(),
                            kind: BlockKind::Content,
                            reserved_rows: band.rows,
                            segment_index,
    ...
    /// identifier [`Self::drain_patched_live`] hands out. Each entry newly
    /// elided here is reported once by [`Self::take_elided`].
    fn release_elided(&mut self, blocks: &[CommandBlock]) {
        for (idx, entry) in self.entries.iter_mut().enumerate() {
            ...
            if elided && !entry.emit.bundle.mime.is_empty() {
                ...
                self.elided.push(idx);
            }
        }
    }

    /// Entries whose source block was elided since the last call, as
    /// `(block_index, segment_index)` pairs naming the rendered block to
    /// drop. Each elision is reported exactly once.
    pub fn take_elided(&mut self) -> Vec<(usize, usize)> {
        std::mem::take(&mut self.elided)
            .iter()
            .filter_map(|&idx| {
                self.entries
                    .get(idx)
                    .map(|e| (e.block_index, e.segment_index))
            })
            .collect()
    }
    ...
    /// Move every entry anchored at or below absolute `row` by `delta` rows:
    /// a band above them grew or shrank, and their anchors must keep pointing
    /// at the same content for the next placement pass. Signed, because a band
    /// hands rows back as well as taking them.
    pub fn shift_rows_at_or_below(&mut self, row: usize, delta: isize) {
        for entry in &mut self.entries {
            if entry.abs_row >= row {
                entry.abs_row = entry.abs_row.saturating_add_signed(delta);
            }
        }
    }

    /// Push every entry's anchor through a resize reflow's row remap (see
    /// [`winter_render::RowRemap`]): the live screen was rebuilt by replaying
    /// logical lines, so a band's rows moved with the re-wrapped content
    /// above it.
    pub fn remap_rows(&mut self, remap: &winter_render::RowRemap) {
        for entry in &mut self.entries {
            entry.abs_row = remap.map(entry.abs_row);
        }
    }
...
mod tests {
    ...
    /// A default-height band starting at `abs_row`, for the update calls below:
    /// these tests are about anchoring and trust, not band heights.
    fn band(abs_row: usize) -> BlockBand {
        BlockBand {
            abs_row,
            rows: TEST_BAND_ROWS,
        }
    ...
    /// Arbitrary band height for the fixtures above.
    const TEST_BAND_ROWS: usize = 4;
    ...
    fn test_wire_requested_trust_is_clamped_to_the_default_ceiling() {
        ...
        queue.update(term.scrollback(), band(0), &[]);
    ...
    fn test_update_surfaces_live_blocks() {
        ...
        let mut queue = BlockQueue::new();
        queue.update(term.scrollback(), band(5), &[]);
        assert_eq!(queue.entries().len(), 1);
        assert_eq!(queue.entries()[0].kind, BlockKind::Live);
        assert_eq!(queue.entries()[0].abs_row, 5);
    }
    ...
    fn test_shift_rows_at_or_below_moves_only_later_entries() {
        ...
        let mut queue = BlockQueue::new();
        queue.update(term.scrollback(), band(0), &[band(2), band(5)]);
        queue.shift_rows_at_or_below(5, 3);

        assert_eq!(queue.entries()[0].abs_row, 2);
        assert_eq!(queue.entries()[1].abs_row, 8);
    }
    ...
    #[test]
    fn test_take_elided_reports_each_elided_entry_exactly_once() {
        // Regression: the retention budget freed the entry's MIME bundle but
        // never told the owner, so the rendered block (GPU texture, WebView
        // tile) stayed alive forever with nothing left to show.
        let mut term = Terminal::new();
        // One command block carrying a content segment, delimited by OSC 133
        // marks so later output lands in *other* blocks and the budget can
        // elide this one.
        term.feed(b"\x1b]133;A\x1b\\$ \x1b]133;B\x1b\\\x1b]133;C\x1b\\");
        term.feed(&emit_escape(&svg_emit()));
        term.feed(b"\x1b]133;D;0\x1b\\\n");

        let mut queue = BlockQueue::new();
        queue.update(term.scrollback(), band(0), &[]);
        assert!(
            queue.take_elided().is_empty(),
            "nothing is elided before the budget is exceeded"
        );

        // Stream many command blocks past the retention budget (~8 MiB) so
        // the oldest block, the one holding the SVG, gets elided.
        let chunk = "x".repeat(64 * 1024);
        for _ in 0..160 {
            term.feed(b"\x1b]133;A\x1b\\$ \x1b]133;B\x1b\\\x1b]133;C\x1b\\");
            term.feed(chunk.as_bytes());
            term.feed(b"\x1b]133;D;0\x1b\\\n");
        }
        queue.update(term.scrollback(), band(0), &[]);

        let elided = queue.take_elided();
        assert_eq!(elided.len(), 1, "exactly the SVG block's entry");
        assert_eq!(elided[0].0, 0, "it belongs to block 0");
        assert!(
            queue.entries()[0].emit.bundle.mime.is_empty(),
            "the elided entry keeps no payload"
        );
        assert!(
            queue.take_elided().is_empty(),
            "an already-reported elision must not re-report"
        );
    }
}
