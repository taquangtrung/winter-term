...
impl GpuRenderer {
    ...
    fn pane_cell_extent(&self, rect: PaneRect) -> PaneExtent {
        PaneExtent {
            cols: ((rect.width - crate::PANE_H_PAD * 2.0) / self.cell_width).floor() as usize,
            rows: (rect.height / self.cell_height).floor() as usize,
        }
    }
    ...
    fn push_pane_background(
        ...
        let grid = pane.grid;
        let rect = pane.rect;
        let extent = self.pane_cell_extent(rect);
            ...
            for col in 0..grid.cols().min(pane_cols) {
                ...
                if let Some(scale) = quad_scale {
                    ...
                    let mut span_attrs = Attrs::new()
                        .family(base_family(style.family.as_deref()))
                        .color(self.dim_text(text_fg.0, text_fg.1, text_fg.2, pane_dim));
    ...
    /// Blend one RGB triple toward the theme background, which is how an
    /// inactive pane's text is de-emphasized. A no-op when `dim` is false.
    fn dim_text(&self, r: u8, g: u8, b: u8, dim: bool) -> Color {
        if !dim {
    ...
    fn push_fallback_glyph_quad(&mut self, glyph: FallbackGlyphQuad, layers: &mut FrameLayers) {
        ...
        let color = if let Some(label_color) = glyph.label_color {
            ...
            self.dim_text(r, g, b, glyph.dim)
        } else {
            match glyph.cell.map(|c| c.style.foreground) {
                Some(GridColor::Rgb(rgb)) => self.dim_text(rgb.r, rgb.g, rgb.b, glyph.dim),
                ...
                None | Some(GridColor::Default) => {
                    ...
                    self.dim_text(fg.r, fg.g, fg.b, glyph.dim)
                }
            }
        };
    ...
    fn cell_glyph_attrs<'a>(
        ...
        if let Some((r, g, b)) = context.bracket_rgb {
            ...
            attrs = attrs.color(self.dim_text(fg.0, fg.1, fg.2, context.dim));
        } else if text_color_explicit {
            attrs = attrs.color(self.dim_text(text_fg.0, text_fg.1, text_fg.2, context.dim));
        }
    ...
    fn window_border_verts(&self, surface: SurfaceSize) -> Vec<BgVertex> {
    ...
    fn upload_vertex_buffers(
        ...
        let border_verts = self.window_border_verts(surface);
